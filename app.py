...
def get_match_id_from_request(handler):
...
def get_current_match(match_id=None):
    """Get current match data by MatchID"""
    if not match_id:
        match_id = STATE["current_match_id"]
    
    if not match_id or match_id not in STATE["matches"]:
        # Create new match if none exists
        match_id = create_new_match()
    
    return STATE["matches"][match_id], match_id


def update_match_timestamp(match_id):
    """Update the last updated timestamp for a match"""
    if match_id in STATE["matches"]:
        STATE["matches"][match_id]["updated_at"] = datetime.now().isoformat()


def get_all_matches():
    """Get list of all matches with basic info"""
    matches_list = []
    for match_id, match_data in STATE["matches"].items():
        matches_list.append({
            "match_id": match_id,
            ...
            "team_a_name": match_data["match"].team_a.team_name if match_data["match"].team_a else None,
            "team_b_name": match_data["match"].team_b.team_name if match_data["match"].team_b else None,
        })
    return sorted(matches_list, key=lambda x: x["updated_at"], reverse=True)
...
<head>
  ...
  <title>CricketSmart-Match - Live Cricket Scoring</title>
  <style>
    body { font-family: Arial, sans-serif; padding: 20px; background-color: #01147C; color: white; }
    .muted { color: #b0c4de; font-size: 14px; }
    .error { color: #ff6b6b; margin-top: 10px; }
    .score { font-size: 24px; font-weight: bold; margin-bottom: 10px; }
    .row { display: flex; gap: 20px; }
    .card { background: rgba(255, 255, 255, 0.95); color: #333; padding: 20px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1); margin-bottom: 20px; }
    .controls { display: flex; gap: 8px; flex-wrap: wrap; }
    .controls button { padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer; background: #007bff; color: white; font-weight: 500; transition: all 0.2s ease; }
    .controls button:hover { background: #0056b3; transform: translateY(-1px); box-shadow: 0 2px 8px rgba(0, 123, 255, 0.3); }
    ...
    @media (max-width: 480px) {
      ...
      /* Summary tabs mobile fix */
      div[style*="display:flex;border-bottom:2px solid #ddd"] {
        flex-wrap: wrap !important;
        gap: 2px !important;
      }
      div[style*="display:flex;border-bottom:2px solid #ddd"] button {
    ...
    .tab { padding: 8px 16px; margin-right: 8px; background: rgba(255, 255, 255, 0.2); border: 1px solid rgba(255, 255, 255, 0.3); border-bottom: none; cursor: pointer; border-radius: 8px 8px 0 0; color: white; transition: all 0.2s ease; }
    .tab.active { background: rgba(255, 255, 255, 0.95); color: #01147C; border-bottom: 1px solid rgba(255, 255, 255, 0.95); position: relative; top: 1px; }
    .tabContent { border: 1px solid rgba(255, 255, 255, 0.3); padding: 20px; border-radius: 12px; background: rgba(255, 255, 255, 0.95); color: #333; }
    input, select { padding: 8px 12px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; transition: border-color 0.2s ease; }
    input:focus, select:focus { outline: none; border-color: #007bff; box-shadow: 0 0 0 2px rgba(0, 123, 255, 0.2); }
    button { padding: 10px 20px; border: none; border-radius: 6px; cursor: pointer; font-weight: 500; transition: all 0.2s ease; }
    .add-player-btn { background: #01147C; color: white; width: 100%; padding: 10px; font-weight: bold; border: 2px solid rgba(255, 255, 255, 0.2); }
    .add-player-btn:hover { background: #0152cc; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(1, 20, 124, 0.3); }
    h1 { color: white; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.3); }
    h2, h3, h4 { color: #01147C; }
    hr { border: none; height: 1px; background: linear-gradient(to right, transparent, rgba(255, 255, 255, 0.3), transparent); margin: 20px 0; }
    #teamAList, #teamBList { background: rgba(1, 20, 124, 0.05); padding: 10px; border-radius: 6px; border: 1px solid rgba(1, 20, 124, 0.1); }
    #teamAList li, #teamBList li { padding: 4px 0; border-bottom: 1px solid rgba(1, 20, 124, 0.1); }
    #teamAList li:last-child, #teamBList li:last-child { border-bottom: none; }
    .popup-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); backdrop-filter: blur(5px); z-index: 1999; display: flex; align-items: center; justify-content: center; padding: 20px; }
    .popup-alert { background: white; color: #333; padding: 20px; border-radius: 12px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3); max-width: 90%; width: 90%; text-align: center; animation: popupSlide 0.3s ease-out; }
    .popup-alert h3 { color: #01147C; margin: 0 0 15px 0; font-size: 18px; }
    .popup-alert p { margin: 0 0 20px 0; line-height: 1.5; font-size: 14px; }
    .popup-alert button { background: #01147C; color: white; padding: 12px 24px; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 16px; min-height: 44px; }
    .popup-alert button:hover { background: #0152cc; }
    @keyframes popupSlide { from { transform: scale(0.8); opacity: 0; } to { transform: scale(1); opacity: 1; } }
    
    /* 3D Trophy Animations for Match Summary */
    .trophy-3d {
      ...
      padding: 6px 12px;
      margin-right: 12px;
      box-shadow: 0 6px 12px rgba(30, 60, 114, 0.3), inset 0 3px 6px rgba(255, 255, 255, 0.1);
      position: relative;
    }
    ...
    .trophy-stars {
      ...
      animation: starsOrbitHorizontal 4s linear infinite;
    }
...
<body>
    ...
    <div id="matchHeader" style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 8px; margin-bottom: 20px; display: none;">
      <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
        ...
        <div style="display: flex; gap: 10px; align-items: center;">
          <button onclick="shareMatch()" style="background: #28a745; color: white; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer; font-weight: bold;">🔗 Share Match</button>
          <button onclick="showMatchesList()" style="background: #17a2b8; color: white; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer;">📋 All Matches</button>
          <button onclick="goToHome()" style="background: #ffc107; color: black; padding: 8px 16px; border: none; border-radius: 6px; cursor: pointer; font-weight: bold;">🏠 Home</button>
        </div>
      </div>
    </div>
  ...
  <div id="matchSettingsTab" class="tabContent" style="display: none;">
    ...
    <div style="margin-top:20px">
      <button onclick="showPlayerSetup()" style="background:#28a745;color:white;padding:10px 20px;border:none;border-radius:5px;cursor:pointer;font-weight:bold;">Next: Add Players →</button>
    </div>
    ...
    <div id="playerSetupSection" style="display:none;margin-top:30px">
      ...
      <div style="margin-top:15px">
        ...
        <button onclick="backToConfig()" style="background:#ffc107;color:black;padding:10px 20px;border:none;border-radius:5px;cursor:pointer;min-height:44px;flex:1;">← Back to Config</button>
      </div>
    </div>
  </div>
  ...
  <div id="scoreboardTab" class="tabContent" style="display:none;">
    <div class="card">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; flex-wrap: wrap; gap: 10px;">
        ...
        <button id="downloadPDFBtn" onclick="downloadScoreboardPDF()" disabled style="background: #ccc; color: #666; border: none; padding: 10px 20px; border-radius: 5px; cursor: not-allowed; font-size: 14px; min-height: 44px;">📄 Download PDF (Match Not Started)</button>
      </div>
  ...
  <div id="donationSection" style="position: fixed; bottom: 20px; right: 20px; background: rgba(255,255,255,0.95); color: #333; padding: 15px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); max-width: 250px; z-index: 1000;">
    <div style="text-align: center;">
      <h4 style="margin: 0 0 10px 0; color: #01147C; font-size: 14px;">Support CricSmart ❤️</h4>
      ...
      <p style="margin: 0; font-size: 11px; line-height: 1.3;">If you like CricSmart, kindly donate (from ₹10 onwards) for further improvements.</p>
      <div style="margin-top: 10px;">
      ...
      <button onclick="toggleDonation()" style="background: #01147C; color: white; padding: 5px 10px; border: none; border-radius: 4px; cursor: pointer; font-size: 11px; margin-top: 8px;">×</button>
    </div>
  </div>
...
function openUPI() {
  ...
  setTimeout(function() {
    showCustomAlert(
      'Opening UPI app...<br><br>If it doesn&#39;t open automatically:<br><br>1. Copy UPI ID: <strong>' + upiId + '</strong><br>2. Open any UPI app (PayTM, PhonePe, Google Pay, etc.)<br>3. Paste and send minimum ₹10',
      '💸'
    );
  }, 1000);
}
...
function copyUPIFromVictory() {
  ...
  navigator.clipboard.writeText(upiId).then(function() {
    showCustomAlert(
      '🎉 Thank you for your support!<br><br>UPI ID copied: <strong>' + upiId + '</strong><br><br>Your donation helps us improve CricSmart!',
      '📱'
    );
  }).catch(function(err) {
    // Fallback for older browsers
    ...
    document.body.removeChild(textArea);
    showCustomAlert(
      '🎉 Thank you for your support!<br><br>UPI ID copied: <strong>' + upiId + '</strong>',
      '📱'
    );
  });
}
...
function openUPIFromVictory() {
  ...
  setTimeout(function() {
    showCustomAlert(
      '🎉 Thank you for supporting CricSmart!<br><br>If payment doesn&#39;t start automatically:<br><br>1. Copy UPI ID: <strong>' + upiId + '</strong><br>2. Open any UPI app and send ₹10<br><br>Your support means a lot to us!',
      '💸'
    );
  }, 1000);
}
...
function applyViewOnlyRestrictions() {
  ...
  banner.innerHTML = '🔒 View-Only Mode - Spectating | Auto-refreshing every 10 seconds';
...
function showBoundaryCelebration(runs) {
  ...
  popup.innerHTML = 
    ...
    '<div style="font-size:14px;opacity:0.7;margin-top:10px;">🌟 Spectator Celebration 🌟</div>';
  ...
  if (matchId) {
    ...
    popup.innerHTML = 
      ...
      '<div style="background: #e3f2fd; border-left: 4px solid #3b82f6; padding: 15px; margin-bottom: 20px; border-radius: 0 8px 8px 0;">' +
        '<div style="color: #1e40af; font-weight: bold; margin-bottom: 10px;">🔒 Secure View-Only Access</div>' +
      ...
      '<div style="display: flex; gap: 10px; justify-content: center;">' +
        '<button onclick="copyShareLink(\\'' + shareUrl + '\\')" style="background: #007bff; color: white; padding: 10px 20px; border: none; border-radius: 8px; cursor: pointer; font-weight: bold;">📋 Copy Link</button>' +
    ...
    showPopupAlert('No Active Match', 'Please create a new match first before sharing. Click the "➕ New Match" button to get started.');
  }
}
    ...
    const matchesList = matches.map(match => 
      '<div style="border: 1px solid #ddd; border-radius: 8px; padding: 15px; margin-bottom: 10px; background: white;">' +
        ...
        '<div style="font-size: 14px; margin-bottom: 8px;">' +
          ...
          (match.started ? ' 🏏 Started' : ' ⏳ Not Started') +
        '</div>' +
...
async function refresh() {
  ...
  for (const p of st.team_a_players) {
    if (ulA) {
      ...
      playerInfo.textContent = p.name + ' (' + p.role + ')' + (st.team_a_captain === p.name ? ' 🧢' : '') + getPlayerRoleIcon(p.role);
  ...
  for (const p of st.team_b_players) {
    if (ulB) {
      ...
      // Player info
      const playerInfo = document.createElement('span');
      playerInfo.textContent = p.name + ' (' + p.role + ')' + (st.team_b_captain === p.name ? ' 🧢' : '') + getPlayerRoleIcon(p.role);
  ...
  if (st.started) {
    ...
    if (summaryBallDiv1st) {
      ...
      if (first_innings_ballbyball.length > 0) {
        ...
        inningsHeader.innerHTML = '<h4 style="color:#2196f3;margin-bottom:10px;">1st Innings Ball by Ball ✓ COMPLETED</h4>';
    ...
    if (summaryBallDiv2nd) {
      ...
      if (second_innings_ballbyball.length > 0) {
        ...
        inningsHeader.innerHTML = '<h4 style="color:#2196f3;margin-bottom:10px;">2nd Innings Ball by Ball' + (isSecondInningsComplete ? ' ✓ COMPLETED' : '') + '</h4>';
    ...
    if (summaryBallDivCombined) {
      ...
      if (first_innings_ballbyball.length > 0) {
        const inningsHeader = document.createElement('div');
        inningsHeader.innerHTML = '<h4 style="color:#2196f3;margin:15px 0 10px 0;">1st Innings Ball by Ball ✓ COMPLETED</h4>';
      ...
      if (second_innings_ballbyball.length > 0) {
        const inningsHeader = document.createElement('div');
        const isSecondInningsComplete = st.summary && st.summary.innings > 2;
        inningsHeader.innerHTML = '<h4 style="color:#2196f3;margin:15px 0 10px 0;">2nd Innings Ball by Ball' + (isSecondInningsComplete ? ' ✓ COMPLETED' : '') + '</h4>';
        summaryBallDivCombined.appendChild(inningsHeader);
  ...
  if (st.match_complete) {
    ...
    if (matchWinnerLine && st.match_winner) {
      matchWinnerLine.innerHTML = `🏆 Winner: ${st.match_winner}`;
    ...
    if (scoreboardWinnerLine && st.match_winner) {
      scoreboardWinnerLine.innerHTML = `🏆 Winner: ${st.match_winner}`;
...
function createInningsCard(st, battingTeam, inningsNumber) {
  ...
  battingPlayers.forEach((player, index) => {
    ...
    html += '<tr style="background:' + bg + '">' +
      '<td style="padding:10px;border:1px solid #ddd;font-weight:bold">' + (player.name || 'Unknown') + getPlayerRoleIcon(player.role) + (isCaptain ? ' 🧢' : '') + '</td>' +
  ...
  bowlingPlayers.forEach((bowler, index) => {
    ...
    html += '<tr style="background:' + bg + '">' +
      '<td style="padding:10px;border:1px solid #ddd;font-weight:bold">' + (bowler.name || 'Unknown') + getPlayerRoleIcon(bowler.role) + (isBowlerCaptain ? ' 🧢' : '') + '</td>' +
...
function showPlayerNameDialog(message) {
  return new Promise((resolve) => {
    ...
    popup.innerHTML = `<div style="font-size: 36px; margin-bottom: 15px;">👤</div>
...
function disableScoringControls() {
  ...
  matchCompleteDiv.textContent = '🏆 Match Completed! Use Restart button to start a new match.';
...
function disableScoringForInningsComplete() {
  ...
  if (scoringDiv && scoringDiv.querySelector('h2')?.textContent === 'Scoring') {
    ...
    if (!existingMsg) {
      ...
      msgDiv.textContent = '🏏 Innings Complete! Click "Switch Innings" to continue with 2nd innings.';
...
function showSwitchInningsTooltip() {
  ...
  tooltip.style.cssText = 
    ...
    'pointer-events: none;';
  tooltip.innerHTML = '🎯 Click here to start 2nd innings!';
...
function showVictoryPopup(winner) {
  ...
  popup.innerHTML = 
    '<div style="font-size:48px;margin-bottom:20px;animation:trophyBounce 1s ease-in-out infinite alternate;">🏆</div>' +
    ...
    '<div style="font-size:18px;margin-bottom:30px;">Congratulations on a fantastic match! 🎉</div>' +
    '<div style="background:rgba(255,255,255,0.1);padding:20px;border-radius:15px;margin-bottom:25px;">' +
    '<div style="font-size:16px;margin-bottom:15px;font-weight:bold;">🎯 Enjoyed CricSmart?</div>' +
    ...
    '<button onclick="copyUPIFromVictory()" style="background:#01147C;color:white;padding:8px 15px;border:none;border-radius:20px;cursor:pointer;font-size:14px;font-weight:bold;">📱 Copy UPI</button>' +
    '<button onclick="openUPIFromVictory()" style="background:#28a745;color:white;padding:8px 15px;border:none;border-radius:20px;cursor:pointer;font-size:14px;font-weight:bold;">💸 Donate ₹10</button>' +
    '</div>' +
    '<div style="font-size:12px;color:rgba(255,255,255,0.8);">UPI: vasudevant25@okicici</div>' +
    '</div>' +
    '<div style="display:flex;gap:15px;justify-content:center;margin-bottom:20px;">' +
    '<button onclick="downloadScoreboardPDF(); this.parentElement.parentElement.remove();" style="background:#01147C;color:white;padding:12px 25px;border:none;border-radius:25px;cursor:pointer;font-size:16px;font-weight:bold;animation:buttonGlow 2s ease-in-out infinite;">📄 Download PDF</button>' +
...
function updateDownloadButton(st) {
  ...
  if (!st.started) {
    ...
    downloadBtn.textContent = '📄 Download PDF (Match Not Started)';
    ...
    downloadBtn.textContent = '📄 Download PDF (Match Completed)';
  } else {
    ...
    downloadBtn.style.color = 'white';
    downloadBtn.style.cursor = 'pointer';
    downloadBtn.textContent = '📄 Download PDF (Live Match)';
  }
}
